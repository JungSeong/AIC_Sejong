...
사용법:
  ...
  python3 collect_data.py --seed 42                     # 재현 가능한 시드 (단일)
  python3 collect_data.py --seed-start 0   --seed-end 99   # 협업 A: 시드 0~99 (100 세트)
  python3 collect_data.py --seed-start 100 --seed-end 199  # 협업 B: 시드 100~199 (100 세트)
...
def _make_nic_trial(nic_rail: int, diversify: bool) -> dict:
    ...
    # [수정 이유] 시나리오 생성 시 결정된 그라운드 트루스 파지 오프셋(±2mm 노이즈 포함)을 
    # 정책 노드에서 기록할 수 있도록 환경변수로 전달함.
...
def _make_sc_trial(sc_rail: int, diversify: bool) -> dict:
    ...
    # [수정 이유] SC trial에서도 시나리오 생성 시 결정된 파지 오프셋을 정책 노드에 전달함.
    os.environ["AIC_CAPTURE_GRIPPER_OFFSET_X"] = f"{offset_x:.6f}"
    os.environ["AIC_CAPTURE_GRIPPER_OFFSET_Y"] = f"{offset_y:.6f}"
    os.environ["AIC_CAPTURE_GRIPPER_OFFSET_Z"] = f"{offset_z:.6f}"
...
def start_policy(
    ...
    # [수정 이유] 시나리오 생성 시 결정된 Ground Truth Gripper Offset 값을 
    # 정책 노드(perturbcollect)가 읽을 수 있도록 환경변수 사본에 확실히 포함함.
    # 또한 정책을 'datacollect'에서 'perturbcollect'로 변경하여 확장된 기록 로직을 활성화함.
    ...
    print(f"[Policy] PerturbCollect 시작. 저장 경로: {capture_dir} / {step_hz}Hz")
...
def run_collection_loop(
    ...
    # Hub 업로드 경로: 지정하지 않으면 실행 시작 시각을 기반으로 자동 생성.
    # 같은 사용자(repo_id)의 서로 다른 Trial이 각자의 서브폴더에 독립 적재됩니다.
    #   예) aic-sejong-team/A  →  runs/20260417_171600/
    #       aic-sejong-team/A  →  runs/20260417_190000/   ← 두 번째 실행
    #       aic-sejong-team/B  →  runs/20260417_171600/   ← B는 별개 레포
    if hub_path_in_repo is None:
        run_id = datetime.now().strftime("%Y%m%d_%H%M%S")
        hub_path_in_repo = f"runs/{run_id}"

    # 협업 범위 모드: seed_start 기반으로 세트별 시드를 직접 매핑
    ...
    for set_idx in range(1, n_sets + 1):
        ...
        if hub_repo_id and not dry_run:
            ...
            if not hub_private:
                # convert_to_lerobot.py에 public 옵션이 필요하다면 추가 가능 (현재는 기본 private)
                pass
...
def main():
    parser = argparse.ArgumentParser(
        ...
        python3 collect_data.py --seed 42                     # 단일 시드 고정 (재현용)
        python3 collect_data.py --gazebo-wait 60              # Gazebo 초기화 대기 60초
        python3 collect_data.py --headless                    # Gazebo & RViz GUI 없이 백그라운드 실행
        python3 collect_data.py --hub-repo-id                 # 수집한 데이터 세트를 올릴 리포지토리 선택

        협업 모드 (시드 범위 분할):
        python3 collect_data.py --seed-start 0   --seed-end 99   # A: 시드 0~99   (100 세트)
        python3 collect_data.py --seed-start 100 --seed-end 199  # B: 시드 100~199 (100 세트)
        python3 collect_data.py --seed-start 200 --seed-end 299  # C: 시드 200~299 (100 세트)
""",
    )
    ...
    parser.add_argument("--hub-path-in-repo",  type=str,  default=None,
                        help="Hub 레포 내 저장 경로 prefix."
                             ...
                             " 같은 사용자의 여러 Trial이 동일 레포 내 별도 서브폴더에 누적됩니다.")

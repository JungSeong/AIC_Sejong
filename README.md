...
```
1. YOLO Keypoint Detection 및 Multiview Triangulation 기반 포트 3-DoF 추론 로직 구현, 10개 Case에서 오차 범위 XX% 이내로 예측
2. 케이블 정렬을 위한 Billinear-CNN 모델 기반 Fine-Grained Model 모델 구현, Baseline 대비 MAE XX% 향상
3. Gazebo Simulator 기반 데이터 수집 노드 (LeRobot.py, PortOffsetCollect.py) 구현
```

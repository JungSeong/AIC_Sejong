...
class MultimodalPoseRegressor(nn.Module):
    """Predict both SFP port offsets and one yaw correction from image/FT/joint inputs.

    The model has three task heads:
    - ``port0_position``: ``[dx, dy]``
    - ``port1_position``: ``[dx, dy]``
    - ``dyaw``: one scalar yaw correction
    """
    ...
    def __init__(
        ...
        *,
        ...
        image_feature_dim = feature_dim * self.num_views if aggregation == "concat" else feature_dim
    ...
    def _encode_image(self, image: torch.Tensor) -> torch.Tensor:
        ...
        if image.ndim != 5:
            raise ValueError(f"Expected image tensor with 4 or 5 dims, got {tuple(image.shape)}")

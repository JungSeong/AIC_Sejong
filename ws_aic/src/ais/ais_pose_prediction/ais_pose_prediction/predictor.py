...
from .config import PosePredictionConfig
...
class PosePredictor:
    def __init__(
        self,
        *,
        checkpoint_path: str | Path = PosePredictionConfig.CHECKPOINT_PATH,
        device: str = PosePredictionConfig.DEVICE,
        logger=None,
    ) -> None:
        self.logger = logger
        ...
        self.cameras = PosePredictionConfig.CAMERAS
    ...
    def _load(self) -> None:
        if not self.checkpoint_path.is_file():
            raise FileNotFoundError(f"Pose prediction checkpoint not found: {self.checkpoint_path}")
    ...
    def _joint_positions(self, observation) -> torch.Tensor:
        ...
        values = torch.tensor([float(by_name.get(name, 0.0)) for name in JOINT_NAMES], dtype=torch.float32)
    ...
    def predict(self, observation) -> Optional[dict[str, np.ndarray | float]]:
        ...
        if not np.isfinite(port0_mm).all() or not np.isfinite(port1_mm).all() or not np.isfinite(dyaw):

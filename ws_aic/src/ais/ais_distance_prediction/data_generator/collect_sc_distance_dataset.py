#!/usr/bin/env python3
"""Collect SC distance-prediction samples near the 99 percent YOLO pose region.

Output layout:
  ws_aic/data/distance_prediction/SC/
    samples.jsonl
...
class ScDistanceCollector(Node):
    def __init__(self) -> None:
        super().__init__("sc_distance_dataset_collector")
        ...
        self.get_logger().info("SC distance dataset collector ready.")
    ...
    def log_tf_hints(self, missing_frame: str) -> None:
        ...
        if not hints:
            self.get_logger().error(
                f"Missing TF: {missing_frame}. No task_board/SC frames are visible."
            )
            return
        self.get_logger().error(
    ...
    def port_frame(self, module_name: str, port_name: str, mode: str = DEFAULT_PORT_FRAME_MODE) -> str:
        ...
        if (
            mode == "auto"
            and port_name == "sc_port_base"
            and self.wait_for_tf("base_link", f"task_board/{module_name}/sc_port_link", timeout_s=0.5)
        ):
            return f"task_board/{module_name}/sc_port_link"
        return base_frame
    ...
    def discover_module_name(
        ...
        module_names = self.discover_module_names(max_mounts, port_names, port_frame_mode)
    ...
    def discover_module_names(
        ...
        for mount_idx in range(max_mounts):
            module_name = f"sc_port_{mount_idx}"
    ...
    def save_sample(
        ...
        record = {
            ...
            "task_id": f"distance_sc_{module_name}_{target_port_name}",
            "task_type": "sc",
            "port_type": "sc",
            ...
            "label": {
                ...
                "coordinate": "plug_tip position expressed in the target SC port frame",
                "source": "tf_base_link",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--max-mounts", type=int, default=2)
    parser.add_argument("--port-names", type=parse_csv, default=SC_PORT_NAMES)
    parser.add_argument("--all-port-names", type=parse_csv, default=SC_PORT_NAMES)
    parser.add_argument(
        "--port-frame-mode",
        ...
        help=(
            ...
            "and also supports task_board/<module>/sc_port_link for SC targets."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Resume an existing run by appending to samples.jsonl and skipping "
            "already saved step_index values for each module/port. If --run-id "
            "is omitted, the latest run id is inferred from samples.jsonl."
        ),
    )
...
def wait_for_camera_data(node: ScDistanceCollector, wait_s: float) -> bool:
...
def warmup_tf(node: ScDistanceCollector, duration_s: float) -> None:
...
def settle(node: ScDistanceCollector, duration_s: float) -> None:
    start = time.time()
    while time.time() - start < duration_s:
        rclpy.spin_once(node, timeout_sec=0.1)


def log_info(node: ScDistanceCollector, message: str) -> None:
    node.get_logger().info(message)
    print(f"[SC_DISTANCE] {message}", flush=True)
...
def write_generation_config(
    ...
    config = {
        ...
        "source_notebook": "ws_aic/src/ais/ais_yolo_train/notebook/visualize_triangulation_error_SC.ipynb",
        ...
        "frames": {
            ...
            "requested_offset": (
                ...
                else "auto: entrance if available, else link; sc_port_base may resolve to sc_port_link"
            ),
            "label": "same as requested_offset",
            "plug_tip": "resolved cable tip TF, usually cable_1/sc_tip_link",
        },
        "coordinate_convention": (
            "requested_port_local_offset and label.plug_tip_to_port are both "
            "the plug tip position expressed in the selected SC port frame. "
...
def run_id_from_record(record: dict[str, Any]) -> str | None:
    module_name = record.get("target_module_name")
    port_name = record.get("port_name")
    episode_name = record.get("episode_name")
    if not isinstance(module_name, str) or not isinstance(port_name, str):
        return None
    if not isinstance(episode_name, str):
        return None
    suffix = f"_{safe_name(module_name)}_{safe_name(port_name)}_distance_ci99"
    if not episode_name.endswith(suffix):
        return None
    return episode_name[: -len(suffix)]


def iter_sample_records(samples_path: Path) -> list[dict[str, Any]]:
    ...
    with samples_path.open("r", encoding="utf-8") as f:
        for line_number, line in enumerate(f, start=1):
            ...
            if not isinstance(record, dict):
                raise ValueError(f"Invalid sample record in {samples_path}:{line_number}: expected object")
            records.append(record)
    return records


def infer_latest_run_id(samples_path: Path) -> str:
    for record in reversed(iter_sample_records(samples_path)):
        run_id = run_id_from_record(record)
        if run_id:
            return run_id
    raise ValueError(f"Could not infer --run-id from {samples_path}")


def load_completed_step_indices(
    samples_path: Path,
    run_id: str,
) -> dict[tuple[str, str], set[int]]:
    completed: dict[tuple[str, str], set[int]] = {}
    for record in iter_sample_records(samples_path):
        if run_id_from_record(record) != run_id:
            continue
        module_name = record.get("target_module_name")
        port_name = record.get("port_name")
        ...
        if not isinstance(step_index, int):
            continue
        completed.setdefault((module_name, port_name), set()).add(step_index)
    return completed
...
def validate_resume_config(
    ...
    if mismatches:
        raise ValueError(
            ...
            "Use the same sampling arguments, or choose a new --run-id/--output.\n"
            + "\n".join(mismatches)
        )
...
def collect_for_port(
    ...
    if skipped:
        log_info(
            node,
            f"Resuming {module_name}/{port_name}: skipping "
            f"{skipped}/{len(offsets)} already saved samples.",
        )
    ...
    for step_index, offset in enumerate(offsets):
        ...
        for attempt in range(args.placement_check_retries + 1):
            ...
            if placement_check_ok(placement_check, args):
                log_info(node, "Placement check OK: " + placement_check_summary(placement_check))
                break
...
def main() -> None:
    ...
    if args.resume:
        run_id = args.run_id or infer_latest_run_id(samples_path)
    else:
    ...
    completed_by_port = (
        load_completed_step_indices(samples_path, run_id) if args.resume else {}
    )
    ...
    try:
        ...
        if args.target_module_name is None:
            module_names = node.discover_module_names(
                args.max_mounts,
                args.port_names,
                args.port_frame_mode,
            )
        else:
            module_names = [args.target_module_name]
        if not module_names:
            node.log_tf_hints("task_board/sc_port_*/sc_port_base_link")
            node.get_logger().error("No SC module TF found.")
            sys.exit(1)
        ...
        log_info(
            ...
            f"modules={module_names}, ports={args.port_names}, "
            f"run_id={run_id}, offsets_per_port={len(offsets)}, "
            f"resume={args.resume}, port_frame_mode={args.port_frame_mode}, "
